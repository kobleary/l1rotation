...
Since $r$ can be learned from the data (following the procedure in [Bai and Ng (2002)](https://onlinelibrary.wiley.com/doi/epdf/10.1111/1468-0262.00273)) we can assume $r$ is known. Next, we're interested in estimating the true set of factors $F$, that compress the data into fewer columns, and true loadings $\Lambda^*$, that show how the factors are related to the original columns. 

## Problem: Rotational Indeterminacy 

However, there is no unique estimate of $F$ and estimate of $\Lambda^*$ that satisfies the equation above. In fact, infinitely many will. This is referred to as rotational indeterminacy and this issue makes interpretation of the loadings matrix tricky. 

To see the problem, let $H$ be any nonsingular $r \times r$ matrix. We can define an estimate $\Lambda^0 = \Lambda^* (H')^{-1}$ and $F^0 = FH$. This tells us that 
...
Hence $X = F^0\Lambda^{0'} = F\Lambda^*$ can be explained identically well by any "rotation" $H$ of the true loadings and factors. Each arbitrary rotation $H$ corresponds to a different but seemingly valid decomposition of $X$ that provides an alternate interpretation of how the factors and variables are related. But we are interested in the true relationship, $\Lambda^*$. So how can we recover the true interpretation?
...
So throughout, we'll continue to maximize sparsity indirectly but more easily by using the $l_1$ norm instead of the $l_0$ norm for the optimization benefits. 
...
$$
The unconstrained objective function becomes slightly more complicated in the general case of more than two factors because we span higher dimensional space than a plane and therefore need to optimize across multiple angles. But the idea remains the same. 
