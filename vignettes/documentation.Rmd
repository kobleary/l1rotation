...
lf$small_loadings_plot + 
  ggplot2::labs(
    ...
    caption = stringr::str_glue('"Small" is defined as loadings less than {round_hn}')
  )
```

Finally, given the testing plot above, we can conclude that factors 1 and 2 are likely local whereas 3 and 4 are not, since they do not have enough "small" loadings (i.e., smaller than a value of $h_n = 1/\log(n) =$ `r round_hn` in the `example_data`).
